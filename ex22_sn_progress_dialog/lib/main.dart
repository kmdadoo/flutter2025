...
import 'package:sn_progress_dialog/sn_progress_dialog.dart';   // sn_progress_dialog
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Ex22 SN Progress Dialog'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  // sn_progress_dialog 패키지를 사용하여 진행 상황 대화 상자를 구현
  // 진행률(%)이 있는 로딩 화면을 만들 때 유용
  Future _normalProgress(context) async {  
    ...
    // 대화 상자를 띄웁니다.
    pd.show(
      max: 100, //  진행률의 최댓값을 100으로 설정
      msg: 'File Downloading...', // 메시지를 표시
      progressBgColor: Colors.transparent,
    );
    for (int i = 0; i <= 100; i++) {
      pd.update(value: i);
      i++;
      // 100밀리초(0.1초)씩 지연시켜 진행률이 서서히 증가하는 효과를 연출
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            MaterialButton(      // sn_progress_dialog 버튼
